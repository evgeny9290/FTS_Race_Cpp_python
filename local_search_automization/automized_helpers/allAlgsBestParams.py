...
def problemCreatorFromCPP(problem_seeds, path, num_workers):
    ...
    cpp_problem_dir_path = os.path.join(os.getcwd(), '..', 'copsimpleai', 'CPP_Problems')

    if not os.path.exists(cpp_problem_dir_path):
        os.makedirs(cpp_problem_dir_path)

    file_names = os.listdir(cpp_problem_dir_path)
    available_problems = []
    run_que = Queue()
